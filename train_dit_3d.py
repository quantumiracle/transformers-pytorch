...
import wandb
import os
from models.diffusion_transformer_3d import dit_mnist
from models.diffusion import DDPM

PROJECT_NAME = 'diffusion_transformer'
RUN_NAME = f'3d_dit_mnist'
...
def train_mnist():
    ...
    lrate = 1e-4
    save_model = False
    save_dir = './data/3d_dit_mnist_outputs400/'
    ...
    for ep in range(n_epoch):
        ...
        for x, c in pbar:
            optim.zero_grad()
            x = x.to(device)
            x = x.unsqueeze(1).repeat(1, n_frames, 1, 1, 1)  # expand/repeat to get temporal dimension (B, C, H, W) -> (B, T, C, H, W)
            c = c.to(device)
            # one-hot encoding
            if USE_ONE_HOT_CLASS:
        ...
        with torch.no_grad():
            ...
            for w_i, w in enumerate(ws_test):
                ...
                x_gen, x_gen_store = ddpm.sample(n_sample, (n_frames, input_c, input_h, input_w), device, guide_w=w, cond=c_i)

                # hack: remove temporal dimension
                x_gen = x_gen[:, 0]  # (B, T, C, H, W) -> (B, C, H, W)
                x_gen_store = x_gen_store[:, :, 0]  # (N, B, T, C, H, W) -> (N, B, C, H, W)
                x = x[:, 0]  # (B, T, C, H, W) -> (B, C, H, W)
