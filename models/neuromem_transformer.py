...
class NeuralMemory(nn.Module):
    ...
    def memory_loss(self, mem, target):
        return None
...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, is_causal = True):
        ...
        self.temporal_memory = NeuralMemory(
            input_dim=dim_head,
    ...
    def forward(self, x, mem: Optional[torch.Tensor]):
        ...
        # TODO check if using new q; if use new q, output needs to truncate [:, :x.shape[1]] to remove memory frames
        ...
        return self.to_out(out), new_mem
...
class NeuromemTransformer(Module):
    def __init__(
        self, 
        num_tokens,
        dim, 
